		...
		$.museum = function(el, options) {
			...
			plugin.show_image = function(i) {
				if (plugin.loaded === false) {
					plugin.container = $('<div>').addClass(plugin.settings.namespace + '-gallery').appendTo($('body')).css({
						'position' : 'absolute',
						'top' : '0',
						'left' : '0',
					...
					plugin.content = $('<div>').addClass(plugin.settings.namespace + '-content').css({
