...
(function($) {
    ...
    $.fn.treeview = function(settings) {
        var dfop =
            {
				...
				getIcon:function(node){
					return this.cbiconpath+this.icons[node.checkstate];
				},
                /**
        ...
        //pre load the icons
        if (dfop.showcheck) {
            for (var i = 0,len=dfop.icons.length; i < len; i++) {
                var im = new Image();
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            if (dfop.showcheck && nd.showcheck) {
                ...
                ht.push("<img  id='", id, "_", nid, "_cb' class='bbit-tree-node-cb' src='", dfop.getIcon(nd), "'/>");
            }
            //a
        ...
        function getck(items, c, fn,fliter) {
			if(typeof fliter=="undefined"){
				fliter=function(){ return true};
			}
            for (var i = 0, l = items.length; i < l; i++) {
				if(!!(items[i].showcheck == true && (items[i].checkstate == 1 ) && fliter(items[i]) )){
					c.push(fn(items[i]));
				}
                if (items[i].ChildNodes != null && items[i].ChildNodes.length > 0) {
                    getck(items[i].ChildNodes, c, fn,fliter);
                }
            }
        }
        function getCkAndHalfCk(items, c, fn,fliter) {
			if(typeof fliter=="undefined"){
				fliter=function(){ return true};
			}
            for (var i = 0, l = items.length; i < l; i++) {
				if(!!(items[i].showcheck == true && (items[i].checkstate == 1 || items[i].checkstate == 2) && fliter(items[i]) )){
					c.push(fn(items[i]));
				}
                if (items[i].ChildNodes != null && items[i].ChildNodes.length > 0) {
                    getCkAndHalfCk(items[i].ChildNodes, c, fn,fliter);
                }
            }
        }
        me[0].t = {
            getSelectedNodes: function(gethalfchecknode,fliter) {
                var s = [];
                if (gethalfchecknode) {
                    getCkAndHalfCk(treenodes, s, function(item) { return item; },fliter);
                }
                else {
                    getck(treenodes, s, function(item) { return item; },fliter);
                }
                return s;
            },
            getSelectedValues: function(gethalfchecknode,fliter) {
                var s = [];
				if (gethalfchecknode) {
                    getCkAndHalfCk(treenodes, s, function(item) { return item.value; },fliter);
                }
                else {
                    getck(treenodes, s, function(item) { return item.value; },fliter);
                }
                return s;
            },
    ...
    $.fn.getCheckedNodes = function(gethalfchecknode,fliter) {
        if (this[0].t) {
            return this[0].t.getSelectedValues(gethalfchecknode,fliter);
        }
        return null;
    };
    $.fn.getTSNs = function(gethalfchecknode,fliter) {
        if (this[0].t) {
            return this[0].t.getSelectedNodes(gethalfchecknode,fliter);
        }
        return null;
    };
